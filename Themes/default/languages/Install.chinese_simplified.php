...
$txt['install_step_writable'] = '读写检测';
$txt['install_step_forum'] = '论坛设置';
$txt['install_step_databaseset'] = '数据库设置';
$txt['install_step_databasechange'] = '数据保存';
$txt['install_step_admin'] = '管理员帐号';
$txt['install_step_delete'] = '结束安装';
...
$txt['install_welcome_desc'] = '欢迎使用SMF，本程序会引导你安装 %1$s ，我们会收集一些简单的配置信息，几分钟之后你就可以使用论坛。';
$txt['install_all_lovely'] = '我们已经完成初始测试并准备好信息，请点击下面的“继续”按钮来开始安装。';
...
$txt['db_settings_info'] = '以下是你数据库服务器需要的设置，如果你不清楚，请咨询服务器提供商。';
$txt['db_settings_type'] = '数据库类型';
$txt['db_settings_type_info'] = '检测到多种数据库类型支持——请选择你希望使用的数据库。';
$txt['db_settings_server'] = '服务器名';
$txt['db_settings_server_info'] = '这里一般都是使用 localhost - 所以，如果你不清楚，请保持使用 localhost 。';
$txt['db_settings_username'] = '用户名';
$txt['db_settings_username_info'] = '请在这里填写连接数据库的用户名。<br />如果你不知道，请尝试你的ftp账号，一般来说可能他们是一样的。';
$txt['db_settings_password'] = '密码';
$txt['db_settings_password_info'] = '这里，请填写连接数据库的密码。<br />如果你不知道，请尝试你的ftp密码。';
$txt['db_settings_database'] = '数据库名称';
$txt['db_settings_database_info'] = '请填写用来存储SMF数据的数据库名称。<br />如果数据库不存在，安装程序将尝试去创建一个。（有权限的前提）';
$txt['db_settings_database_info_note'] = '如果该数据库不存在，安装程序将创建一个新数据库.';
$txt['db_settings_database_file'] = '数据库文件名';
$txt['db_settings_database_file_info'] = '这个是存放SMF数据的文件名。我们推荐你使用随机生成的文件名并且把文件存放在Web服务器公共区域以外的地方。（注意：一般虚拟主机是不允许自己设置名字，请咨询空间商）';
$txt['db_settings_prefix'] = '数据表前缀';
$txt['db_settings_prefix_info'] = '数据库中每个表的前缀。<b>请不要使用同一个前缀来安装两个论坛！</b><br />该值可以允许你在同一个数据库中安装多个论坛。';
$txt['db_sqlite_warning'] = 'Only recommended for small, low volume and/or intranet-type forums';
$txt['db_populate'] = '数据保存';
$txt['db_populate_info'] = '你的设置已经保存并且程序开始根据你的设置写入初始设置到数据库里，写入过程：';
$txt['db_populate_info2'] = '点击“继续”来进入创建管理员页.';
$txt['db_populate_inserts'] = '插入了 %1$d 行数据记录。';
$txt['db_populate_tables'] = '创建了 %1$d 数据表。';
$txt['db_populate_insert_dups'] = '忽略了 %1$d 条重复记录。';
$txt['db_populate_table_dups'] = '忽略了 %1$d 个重复数据表。';
...
$txt['user_settings_skip'] = '忽略';
$txt['user_settings_skip_sure'] = '你确认跳过设置管理员帐号？！';
$txt['user_settings_proceed'] = '完成';

$txt['ftp_checking_writable'] = '检测文件是否可写';
...
$txt['error_db_file'] = '不能找到数据库源脚本！请检查 %s 文件是否在你的论坛根目录。';
$txt['error_db_connect'] = '不能使用你提供的信息来连接数据库服务器。<br /><br />如果你不清楚需要输入什么，请联系你的服务器提供商。';
$txt['error_db_too_low'] = '你的数据库版本非常旧，不能满足SMF的最低要求。<br /><br />请要求你的供应商升级或者提供一个新的，如果他们不能提供，那么请尝试更换供应商。';
$txt['error_db_database'] = '安装程序无法访问 &quot;<i>%s</i>&quot; 数据库。在某些主机，你必须在SMF使用数据库前到管理中心里先创建；某些或必须加上你的前缀——如：你的用户名-你的数据库名。（详细请看你的空间数据库的供应商特别说明）';
$txt['error_db_queries'] = '某些数据库查询没有完全被执行，这有可能是由于是该版本数据库软件不支持（开发版或旧版本）。<br /><br />查询的技术信息：';
$txt['error_db_queries_line'] = '# 行';
$txt['error_db_missing'] = '安装程序无法检测到任何php支持的数据库。请咨询你的主机供应商是否已经把php和数据库一起编译，或加载了正确的扩展插件。';
$txt['error_db_script_missing'] = '安装向导无法找到所有的安装数据库文件，请检查你是否已经上传所有的文件到论坛文件夹，例如 &quot;%1$s&quot;';
...
$txt['error_user_settings_no_password'] = '你的密码至少需要4位或以上。';
...
$txt['error_db_alter_priv'] = '你提供的数据库账号没有权限更改、创建和/或删除数据表；这个对于SMF的功能来说是完全必要的。';
...
$txt['error_mod_security_no_write'] = '安装程序检测到 mod_security 模块已经安装到 Web 服务器。Mod_security 会在 SMF 获得任何信息前拦截提交到表单的数据。SMF 已经内置了一个比 mod_security 更加有效的安全扫描器，并且不会拦截任何表单。<br /><br /><a href="http://www.smf.cn/bbs/index.php/topic,220.0.html">关于禁用 mod_security 的更多信息</a><br /><br />做为选择，你也许想尝试用ftp工具来改变论坛根目录 .htaccess 的权限为可读写(777)，然后刷新本页。';
$txt['error_utf8_version'] = '当前版本的数据库不支持使用 UTF-8 编码集。你可以在去掉使用 UTF-8 选项的情况下安装而不会有任何问题（如果是简体中文，你需要<a href="http://www.smf.cn/downloads/">重新下载utf-8版语言包</a>再重新执行安装）。如果你将来希望使用 UTF-8 (例如：等到数据库服务器升级到 >= %s)，你可以在管理中心里非常方便的升级为 UTF-8 支持。';
$txt['error_valid_email_needed'] = '你输入的Email不正确。';
$txt['error_already_installed'] = '安装程序检测到你已经安装过SMF，强烈要求你<strong>不要</strong>覆盖安装——继续安装<strong>可能会导致数据丢失或不完整</strong>。<br /><br />如果你希望升级论坛，请访问 <a href="http://www.simplemachines.org" target="_blank">Simple Machines 网站（英文）</a> 下载最新的 <em>升级包</em> 。（如有需要，请访问<a href="http://www.smf.cn" target="_blank">SMF中文站</a>寻求支持）<br /><br />如果你希望覆盖原有安装，建议你先删除所有的旧SMF数据库表，替换掉 Settings.php 然后重试。';
$txt['error_warning_notice'] = '警告!';
$txt['error_script_outdated'] = '本安装程序是旧版本，当前最新版本为 %1$s ，但是本安装程序是 %2$s。<br /><br />
	推荐访问 <a href="http://www.simplemachines.org">Simple Machines</a> 网站以保证安装最新版本。';
$txt['error_db_filename'] = '你必须为SQLite输入一个数据库名。';
$txt['error_db_prefix_numeric'] = '你选择的数据库类型不支持数字表前缀。';
$txt['error_invalid_characters_username'] = '用户名使用了非法字符。';
$txt['error_username_too_long'] = '用户名必须少于25个字母。';
$txt['error_username_left_empty'] = '用户名左边有空格。';
$txt['error_db_filename_exists'] = '数据库已经存在，请删除当前的数据文件或者使用其他名称。';
$txt['error_db_prefix_reserved'] = '当前输入的书籍表前缀为保留前缀，请尝试输入其他的前缀。';

$txt['upgrade_upgrade_utility'] = 'SMF 升级程序';
$txt['upgrade_warning'] = '警告!';
$txt['upgrade_critical_error'] = '关键错误!';
...
$txt['upgrade_overall_progress'] = '整体进度';
$txt['upgrade_step_progress'] = '本步骤进度';
$txt['upgrade_time_elapsed'] = '已用时间';
$txt['upgrade_time_mins'] = '分';
$txt['upgrade_time_secs'] = '秒';

$txt['upgrade_incomplete'] = '未完成';
$txt['upgrade_not_quite_done'] = '还没有完成哦!';
$txt['upgrade_paused_overload'] = '升级程序已经暂停一下以防止服务器瘫痪，不用担心，没有什么大的问题的，请点击下面的<label for="contbutt">继续按钮</label>继续下一步就可以。';

$txt['upgrade_ready_proceed'] = '感谢选择升级SMF %1$s 。所有的文件看起来都到位了，我们已经准备好继续步骤。';

$txt['upgrade_error_script_js'] = '升级程序不能找到 script.js 文件或者是这个文件是旧的，请确保风格theme路径是否正确。你可以在 <a href="http://www.simplemachines.org">Simple Machines 网站</a> 下载到一个settings文件的检测的工具，或访问<a href="http://www.smf.cn/bbs/index.php?topic=1043.0" target="_blank">本工具的中文下载页</a>。';

$txt['upgrade_warning_lots_data'] = '安装程序检测到你的论坛有大量的数据需要升级，本向导会根据你的数据量花一点时间来更新，时间的多少是根据你的数据量和服务器状况来定的，如果是超级大的论坛 (~300,000 帖子) 有可能会需要数小时来完成。';
$txt['upgrade_warning_out_of_date'] = '安装程序已经是旧的，当前的SMF版本是 <i id="smfVersion" style="white-space: nowrap;">??</i> ，但是本升级程序是为 <i id="yourVersion" style="white-space: nowrap;">%1$s</i>设计的。<br /><br />建议你访问 <a href="http://www.simplemachines.org">Simple Machines</a> 网站来确保升级程序是最新版本。';
